 ...
 *           17.02.2012          Adjustments to I/O port initialiazations. 
 ***********************************************************************************************************************/
...
void output_ports_configure(void) {
	...
	/*  NOTE:
	 Pins that are present on the 176/177 pin packages but that are not 
	 bonded-out on the smaller packages like the 100-pin LQFP must be 
	 treated as unterminated pins.  To prevent execessive current draw
	 these pins are set to outputs and driven low.
	 */
	...
	/* Port 4 -  */
	/* P40 = SWITCH1                                              : set as input
	 P41 = SWITCH2                                              : set as input
	 P42 = AN002 = Potentiometer to ADC                         : set as analog input
	 P43 = AN003 = Uncommitted                                  : set as low driven output?
	 P44 = SWITCH3                                              : set as input
	 P45 = AN005 = ADC input from SSM2167 Microphone amplifier. : set as analog input
	 P46 = AN006 = Input from RIGHT AUDIO from U14 audio amp.   : set as analog input
	 P47 = AN007 PWMLP-IN from AUDIO FILTER.                    : set as analog input
	 */
	...
	/* Port 6 - This port not bonded out on 100-pin package */
	PORT6.PODR.BYTE = 0x00; /* Set all outputs low */
	PORT6.PDR.BYTE = 0xFF; /* All pins are outputs */

	/* Port 7 - This port not bonded out on 100-pin package */
	PORT7.PODR.BYTE = 0x00; /* Set all outputs low */
	PORT7.PDR.BYTE = 0xFF; /* All pins are outputs */

	/* Port 8 - This port not bonded out on 100-pin package */
	PORT8.PODR.BYTE = 0x00; /* Set all outputs low */
	PORT8.PDR.BYTE = 0xFF; /* All pins are outputs */

	/* Port 9 - This port not bonded out on 100-pin package */
	PORT9.PODR.BYTE = 0x00; /* Set all outputs low */
	PORT9.PDR.BYTE = 0xFF; /* All pins are outputs */
	...
	/* Port F - This port not bonded out on 100-pin package */
	PORTF.PODR.BYTE = 0x00; /* Set all outputs low */
	PORTF.PDR.BYTE = 0x3F; /* All pins are outputs, PF7 & PF6 are reserved */

	/* Port G - This port not bonded out on 100-pin package */
	PORTG.PODR.BYTE = 0x00; /* Set all outputs low */
	PORTG.PDR.BYTE = 0xFF; /* All pins are outputs */

	/* Port J -  WiFi chip select */
	PORTJ.PODR.BYTE = 0x08; /* WiFi CS de-asserted at power up */
	PORTJ.PDR.BYTE = 0x28; /* WiFi CS is an output, PJ5 not bonded-out, others are reserved */
}
